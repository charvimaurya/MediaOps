...
Standalone: `python3 -m agents.remediation_agent [overload|failure|rgb|network]`
Prereqs: KB seeded (python3 -m tools.seed_knowledge_base), ADC configured, Vertex AI enabled.
"""
...
def propose_remediation(
    ...
    *,
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        # Store the precedent context and raw model response with the proposal;
        # this is audit evidence only and grants no authority to execute it.
        ...
        # The structured log summarizes the proposal without replacing the
        # detailed proposal persisted on the incident document.
...
if __name__ == "__main__":
    ...
    if kind not in _DEMO_SUMMARIES:
        print(f"usage: python3 -m agents.remediation_agent [{'|'.join(_DEMO_SUMMARIES)}]")
        sys.exit(2)
