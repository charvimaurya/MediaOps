"""
Checks for incident_cli.py -- the load / prerequisite-check / write-back glue the
per-component CLI runners share.

    python3 -m tests.test_incident_cli

Real Firestore, but a throwaway collection (incidents_manualcli_<ts>) cleaned up
in finally. No Gemini / Vertex calls -- `compute` is a plain fake here; the real
component wiring is proven by the live end-to-end run (see the manual sequence).
"""
